...
const GameOnline = ({ room, user, tag }) => {
  ...
  const gameLog = async(won) => {
        try {
            const response = await fetch(`${url}/api/user/gameLog`, {
                method: 'POST',
    ...
    const updateScore = async (score, won) => {
      console.log('score', user)
        try {
            const response = await fetch(`${url}/api/user/updateScore`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
  ...
  return (
    <div className='boardContainerOnline'>
      ...
      <h2 className='opponent'>
        ...
        <strong style={{fontSize:'large'}}>vs</strong>{' '}
      ...
      {isOpen?<></>:<GameChat room={room} user={user} />}
      
      <button className='updownbutton' onClick={() => setIsOpen(!isOpen)}> {isOpen ? <FaChevronDown /> : <FaChevronUp />} </button>
